...
def reconstruct(model, grid, res, transform):
    ...
    verts, faces, normals, values = measure.marching_cubes(occ, 0)  # 0 if the output of model in range [-1, 1]; 0.5 if the output of model in range [0, 1]
...
# ************************************************************
# eval grid function for our model
def eval_grid(coords, model, num_per_sample=1024, device=torch.device('cuda' if torch.cuda.is_available() else 'cpu')):
    """
    Predict occupancy of values on a grid using the trained model.
    """
    ...
    occ = []
    with torch.no_grad():
        for start_idx in range(0, coords.size(0), num_per_sample):
            end_idx = min(start_idx + num_per_sample, coords.size(0))
            batch_coords = coords[start_idx:end_idx]
            
            # Get model predictions
            batch_occ = model(batch_coords)
            
            # Move predictions to CPU and convert to numpy for processing
            batch_occ = batch_occ.cpu().numpy()
            occ.append(batch_occ)
    
    # Concatenate all batch predictions
    occ = np.concatenate(occ, axis=0)
    return occ.squeeze()  # Make sure the output is 1D
# ************************************************************
...
if __name__ == "__main__":
    ...
    model_path = "./utah_teapot_occupancy_model_100e_singleLoD.pth"
    model = OccupancyModel(grid_size=32, feature_size=16, hidden_size=64)
